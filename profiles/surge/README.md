...
本文档对应当前活动入口 [profiles/surge/surge.conf](surge.conf)，当前版本为 `1.2.2`；权重来源是该入口 `[Proxy Group]` 中 `Smart`、`Smart-Unlimited` 和 `Smart-US` 的实际 `policy-priority`。版本历史见 [CHANGELOG.md](CHANGELOG.md)，节点命名约定见 [docs/operations/surge-smart-node-naming.md](../../docs/operations/surge-smart-node-naming.md)。
...
本文档以 `1.2.2` 活动入口和同版本快照为准；新增节点标签时，先确认它是否能被现有正则命中，再同时更新入口、快照、CHANGELOG 和本说明。不要修改既有 Smart 算法来“简化”文档中的权重；组合规则必须继续放在对应通用规则之前。